...
public class BlockBellEmerald extends Block 
{ 

	public BlockBellEmerald() 
	{
		super(Material.iron); 
		setHardness(3F); 
		setResistance(5F); 
	...
	{this.blockIcon = icon.registerIcon(Infofile.NAME + ":BellEmerald");}
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int id1, float id2, float id3, float id4) 
	{
		...
		player.addPotionEffect(new PotionEffect(Potion.regeneration.id, 100, 10,true));
		return true;
	}
